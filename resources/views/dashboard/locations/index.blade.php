                ...
                <table class="table">
                    ...
                    <tbody>
                        @forelse ($locations as $location)
                            <tr>
                                <td>
                                    <div style="font-weight: 500; color: var(--gray-900);">{{ $location->name }}</div>
                                </td>
                                <td>
                                    <span class="badge badge-primary">{{ $location->unit ?? '-' }}</span>
                                </td>
                                <td>
                                    <code style="background: var(--gray-100); padding: 0.25rem 0.5rem; border-radius: 0.25rem; font-size: 0.875rem;">{{ $location->unit_code ?? '-' }}</code>
                                </td>
                                <td>
                                    <span class="badge badge-secondary">{{ $location->floor ?? '-' }}</span>
                                </td>
                                ...
                                <td>
                                    <div style="display: flex; gap: 0.25rem;">
                                        <a href="{{ route('dashboard.locations.edit', $location) }}" class="btn btn-sm btn-outline-primary" title="Edit">
                                            <span>✏️</span>
                                        </a>
                                        <form action="{{ route('dashboard.locations.destroy', $location) }}" method="POST" style="display: inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus lokasi ini?');">
                                            @csrf
                                            @method('DELETE')
